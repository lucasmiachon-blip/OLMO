#!/usr/bin/env node
/**
 * QA Batch Screenshots — Multi-aula
 *
 ...
 *   node scripts/qa-batch-screenshot.mjs --aula metanalise --act ALL
 *   node scripts/qa-batch-screenshot.mjs --aula cirrose --slide s-a1-01 --video
 *
 * Options:
 *   --aula   cirrose|metanalise|grade (auto-detected from git branch if omitted)
 *   --act    A1|A2|A3|CP|APP|PRE|ALL (default: ALL)
 *   --port   Dev server port (default: from PORT_MAP)
 *   --slide  Single slide ID (overrides --act)
 *   --scale  Device scale factor (default: 2)
 *   --video  Record .webm video per slide (for Gate 4)
 *
 * Output: {aula}/qa-screenshots/{slide-id}/{slide}_{date}_{time}_{state}.png
...
function detectAula(args) {
  const idx = args.indexOf('--aula');
  if (idx >= 0 && args[idx + 1] && PORT_MAP[args[idx + 1]]) return args[idx + 1];
...
const aula = detectAula(argsRaw);
const AULA_DIR = join(SCRIPTS_DIR, '..', aula);
const OUT_BASE = join(AULA_DIR, 'qa-screenshots');
...
if (args.includes('--help') || args.includes('-h')) {
  console.log(`Usage: node scripts/qa-batch-screenshot.mjs [options]

Options:
  --aula <name>  cirrose|metanalise|grade (auto-detected if omitted)
  --act <ACT>    A1|A2|A3|CP|APP|ALL (default: ALL)
  --slide <id>   Single slide ID (overrides --act)
  --port <N>     Dev server port (default: from PORT_MAP)
...
const PAGE_URL = `http://localhost:${PORT}/${aula}/index.html`;

// Import manifest — we need slide order and metadata
const { slides } = await import(pathToFileURL(join(AULA_DIR, 'slides', '_manifest.js')).href);

// Determine which slides to capture
// Supports cirrose acts (A1, A2, CP) AND metanalise phases (F1, I1, F2, I2, F3)
function getActFilter(act) {
  switch (act) {
    ...
    // Generic: match act OR phase (metanalise uses phase: F1, I1, F2, I2, F3)
    default: return s => s.act === act || s.phase === act;
  }
}
