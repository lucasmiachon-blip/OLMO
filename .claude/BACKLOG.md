...
> Counts: P0=3 | P1=8 | Deferred=9 | P2=24 | Frozen=3 | Resolved=17 | Setup=separate. Next #=65.
...
| 63 | infra | M | [S254] SessionStart flags `/insights` + `/dream` bugged — recurring false positives | Lucas observou 4-5x recurrence: `/insights pendente` banner mostra "Xd atras" mesmo quando /insights rodou em sessão recente (provavel `.claude/.last-insights` não escrito no skill close); `/dream` flag (`~/.claude/.dream-pending`) persiste após /dream run. **WORKAROUND aplicado S254:** ambos blocos wrapped em `if false; then ... fi` em `hooks/session-start.sh` (lines 82-90 dream + 107-118 insights). **Reativar via systematic-debugging:** (a) audit `/insights` SKILL.md fim — onde `.last-insights` deveria ser escrito? (b) audit `/dream` SKILL.md fim — onde `.dream-pending` deveria ser deletado? (c) fix lifecycle write-on-close em ambos skills (d) test 1 cycle real (e) un-disable guards (`if false`→`if true`). KBP-07 nota: workaround temporário autorizado por Lucas pq diagnose enfileirado, não silenced. **S256 Block C target.** |
