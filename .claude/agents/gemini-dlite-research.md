---
name: gemini-dlite-research
description: "EXPERIMENTAL S269 D-lite research perna #1. Thin wrapper around .claude/scripts/research-dlite-runner.mjs for Gemini structured-output research. Use only for D-lite benchmark/live smoke; legacy gemini-research.mjs remains canonical until re-bench passes."
tools:
  - Bash
  - Read
  - Grep
...
node .claude/scripts/research-dlite-runner.mjs \
  ...
  --question "<QUESTION>" \
  --out ".claude/.research-tmp/gemini-dlite-<QUESTION_ID>.json"
```
...
- If `GEMINI_API_KEY` is missing, report blocked. Do not substitute WebSearch.
