...
¹ **Caveat (S232 v6):** "GPT-5.4" é Codex plugin marketing label, NÃO OpenAI API model identifier. Distinction: **ChatGPT** (consumer product) ≠ **Codex** (plugin via ChatGPT Plus) ≠ **OpenAI API** (programmatic) ≠ model IDs (ex.: `gpt-4.1-mini`, `gpt-4o`). OLMO não mantém source of truth local para model IDs programáticos desde S232 (purge `config/ecosystem.yaml`); consultar OpenAI docs oficiais (`platform.openai.com/docs/models`) ao invocar API direta.
