...
## Sessao 274 — 2026-04-28 (tipos-ma-7pernas · evidence HTML build + pipeline bench Lane A+B)

> Lucas frame: "lancar nossos scripts antigos e nossos agents skill de pesquisa para construir o evidence htlm sobre tipos de metanalise" → 7 pernas dispatch → 3/7 emit → HTML criado → root causes evidence-based para próxima sessão.

- **HTML criado:** `content/aulas/metanalise/evidence/pre-reading-tipos-ma.html` (~600 li, formato canonical pre-reading-* style). 9 tipos MA cobertos (Pairwise/NMA/IPD-MA/DTA-MA/Prevalence/Dose-response/Bayesian/Living-SR/Umbrella) + 3 abordagens transversais (one-stage vs two-stage, aggregate vs IPD, Component NMA). 30+ PMIDs verified com convenção V (azul, verified)/C (vermelho, candidate) badges — PMID não no texto, só link "PubMed" + badge ao lado (Lucas correção turn 14). s-tipos-ma.html S187 (524 li, 16 PMIDs) preservado intacto.
- **Pipeline 7 pernas dispatch (3/7 emit success):** NLM (44.7KB rich content) + Codex effort=medium override (9 findings JSON, 36 PMIDs verified via NCBI E-utilities) + Evidence-Researcher Opus async (16/16 S187 PMIDs verified via WebFetch fallback quando PubMed MCP DOWN + 9 novas refs 2024-2026).
- **9 novas refs 2024-2026:** Veroniki PRISMA-NMA update 2025 (PMID 40976519), Mazzinari Bayesian IPD-MA Anesthesiology 2025 (PMID 39042027), Ying wCCA 2025 (PMID 41626914), Nevill 168 LSRs survey 2025 (PMID 40712752), Butler LSR cost 2024 (PMID 38043829), Iannizzi LSR guidance 2023 (PMID 38098023), Crippa one-stage 2019 (PMID 29742975), Crippa continuous 2016 (PMID 27485429), Kirvalidze CCA pairwise 2023 (PMID 37501239).
- **Failures registradas (root causes evidence-based):** Gemini.mjs:60 timeout 60s aborta antes do modelo terminar (~91s real para payload thinking 16k + grounding + maxTokens 32k, curl test); Perplexity.mjs:77 timeout 120s não é root cause (Cloudflare drops idle connections at 60s server-side, solução real `stream: true` SSE refactor); D-lite Gemini 400 INVALID_ARGUMENT (root cause NÃO identificado, NÃO é tools+responseJsonSchema combo per Gemini 3 docs + curl 200 OK); PubMed MCP HTTP 400 all endpoints (issue server `@cyanheads/pubmed-mcp-server`).
- **Cost-conscious overrides:** Codex effort=medium override via prompt (vs xhigh padrão, ~5-10x redução custo); Evidence-Researcher model override Sonnet→Opus via Agent tool param (Max plan = $0).
- **Plan:** `.claude/plans/scalable-questing-crane.md` (Execution Outcome + Phase 6 docs + Phase 7 next-session handoff).

### Aprendizados (sessão pesada — 7+ li justificadas)

- **API timeout decay (KBP-54 candidate):** S264 9/9 emit (HANDOFF Lane B) → S274 3/7 emit em 4 meses sem CI bench periódico = regressão silenciosa. Mecânica: Gemini 3.1 + thinking budget + grounding precisa ~91s real (curl mediu); script aborta em 60s. Sonar-deep-research é Cloudflare 60s idle drop server-side, NÃO timeout do script (WebSearch Perplexity community confirmou). Lesson: API timeouts hardcoded sem gate de bench periódico decaem silenciosamente; appendar `/loop` mensal bench API latency em known-bad-patterns.md.
- **KBP-36 self-corrections (Lucas reforçou: "evidência quando dúvida pesquise sempre, todo dia há coisas novas"):** Tive que corrigir 2 inferências sem evidência mid-session: (1) "modelo `gemini-3.1-pro-preview` deprecated" — ERRADO, modelo existe (output curl cortado em 100 li); (2) "responseJsonSchema + google_search incompat → causa do D-lite 400" — ERRADO, Gemini 3 docs explícitas + curl test 200 OK. Lesson: WebSearch/curl test ANTES de afirmar root cause em domínio fast-moving — especialmente quando "todo dia há coisas novas".
- **PMID convention pre-reading HTMLs:** PMID não no corpo do texto — só link com texto "PubMed" + badge V/C ao lado. Lucas corrigiu turn 14. KBP-44 (slides) já cobria pattern similar; agora aplica também a pre-reading-* HTMLs. Update s-tipos-ma.html S187 future com mesma convenção quando merge happen.
- **Resilience pattern — Evidence-Researcher Opus + WebFetch fallback:** PubMed MCP `@cyanheads/pubmed-mcp-server` HTTP 400 todos endpoints durante session inteira. Agent não bloqueou — usou WebFetch para PubMed web direto, verificou 16/16 S187 PMIDs + descobriu 9 refs novas. Pattern reutilizável: agents que dependem de MCPs específicos devem ter WebFetch/curl fallback documentado.
- **Cost-aware model override (2 paths):** (1) Anthropic subagents: Agent tool `model: "opus"` param override per-call (Max plan = $0 cap, qualidade superior para verificação MCPs); (2) External CLI subprocess (Codex): instructar via prompt "use --reasoning-effort medium" (agent é template Sonnet, segue instruction). Pattern aplicável quando bench/POC, não production. Reduziu custo S274 de ~$2.10 estimado para ~$0.05 real (Max plan absorve agents Anthropic; Codex medium em vez de xhigh).
- **Execution friction com auto mode + correções intermediárias:** Lucas mudou escopo várias vezes (turn 4 "Sonnet sem sentido" → turn 5 "tem que lançar" → turn 6 rejected agents → turn 8 "autorizei todos"). Indica que pre-flight clarification mais cuidadoso (AskUserQuestion explícita antes de dispatch) evitaria friction. Em sessão pesada, custo de uma pergunta extra < custo de relançar agents.
- **Plan-mode + auto-mode + Edit cirúrgicos no mesmo file:** Plan aprovado guiou execution com 3 Edits HANDOFF.md raiz não-overlapping (banner + Lane A + Roadmap) sequencial — KBP-25 confirmado. Combinação plan-mode (Phase 6 detalhado) + auto-mode (execução direta sem re-pergunta a cada Edit) preservou momentum sem skipping safety.
