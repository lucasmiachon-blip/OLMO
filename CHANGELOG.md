...
- **`<docs>` docs(S254): update HANDOFF + CHANGELOG + Conductor §6.5 G9 + `.gitattributes`** `[+27/-6, 4 files]` — add Entrega flag disable + S255 priority #3 (test skills/agents) + Caution KBP-07 escape + Aprendizados #4 symptom-vs-root-cause + #5 LOC convention/maturity G9/.gitattributes (3 não-excludentes) + Conductor §6.5 G9 maturity layers SOTA gap + new `.gitattributes` `* text=auto eol=lf` (preventive only, no retroactive renormalize) + commits chain bump 4→6.
...
- **S254-tail conventions (Lucas explicit, 3 não-excludentes)**: (a) **LOC delta** em CHANGELOG commit rows `[+X/-Y, N files]` raw shortstat — adopted starting this commit. CRLF artifacts inflam display (3f488e0 BACKLOG +230/-229 era semantic +1) → caveat trailing quando aplicável. (b) **Maturity layers** (SDL/SAMM/OpenSSF/CMMI) "eram para estar funcionando" mas non-operational → adicionado Conductor §6.5 G9 P2 radar (spec em `docs/research/external-benchmark-execution-plan-S248.md §B5`, sem instrumentation/scoring/gate atual). (c) **`.gitattributes`** criado neste commit (`* text=auto eol=lf`) preventing future CRLF inflation. Retroactive `git add --renormalize .` defer'd para sessão dedicada (touches many files, separate review).
