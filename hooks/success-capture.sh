...
# Parse stdin: extract command + success signal.
# S152 fix: command can be multi-line (heredoc, -m with \n). Strip newlines before
# printing so sed -n '2p' picks up the success flag, not line 2 of the command.
# tool_response has no exit_code field for Bash; interrupted===true is the real
# failure signal.
...
# Gate: skip interrupted invocations
